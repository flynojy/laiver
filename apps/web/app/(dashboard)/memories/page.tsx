              ...
              {Array.isArray(profileSnapshot.by_bucket) ? null : (
                <div className="grid gap-3 md:grid-cols-3">
                  {Object.entries((profileSnapshot.by_bucket as Record<string, unknown>) ?? {}).map(([bucket, value]) => (
                    <div key={bucket} className="rounded-[1rem] border border-[color:var(--border)] bg-[var(--surface-2)] p-4">
            ...
            <CardContent className="space-y-3">
              {conflictGroups.length === 0 ? (
                ...
                conflictGroups.map((group) => (
                  <div key={group.group_id} className="rounded-[1.25rem] border border-[color:var(--border)] bg-[var(--surface)] p-4">
            ...
            <CardContent className="space-y-3">
              {duplicateGroups.length === 0 ? (
                ...
                duplicateGroups.map((group) => (
                  <div key={String(group[0]?.metadata?.dedupe_key ?? group[0]?.id)} className="rounded-[1.25rem] border border-[color:var(--border)] bg-[var(--surface)] p-4">
