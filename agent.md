...
每次接手任务时，先阅读 `project.md` 和本文档，复述当前项目阶段、已知完成度和本轮要解决的问题；先做规划、理解和影响分析，再执行代码或文档修改。

每次完成任务后，必须在最终回复中说明“接下来需要做的工作”，并在 `project.md` 中追加或更新本轮进度备注，便于对话中断后下一次继续。

`project.md` 是当前项目状态和续接上下文的主入口；本文档是 Codex / Agent 协作规则的主入口。若二者出现冲突，先以用户最新要求为准，再同步修正文档。
...
开发过程中优先保持模块解耦：新增能力应放在清晰的 domain service、schema、router 或前端组件边界内，避免把跨领域逻辑塞进既有大文件。若必须改动集中模块，先限定入口和数据契约，后续再拆分。

新增或变更对外接口时，必须同步标注到项目规范或基线文档中，至少说明 endpoint、请求/响应 shape、使用场景和兼容性注意事项；不要只改代码不更新接口说明。
